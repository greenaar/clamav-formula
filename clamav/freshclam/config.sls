...
freshclam_config:
  file.managed:
    ...
    # --version parses the candidate config, then exits before dropping to
    # DatabaseOwner, opening UpdateLogFile, or attempting a database update.
    - check_cmd: /usr/bin/freshclam --version --config-file
    - require:
      - pkg: freshclam_pkg
{% if false %}
freshclam_service:
